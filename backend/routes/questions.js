...
router.post("/questions", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Exactly 4 non-empty options are required." });
  }
  if (
    ...
    return res
      .status(400)
      .json({ message: "Correct answer index must be 1, 2, 3, or 4." });
  }
...
router.put("/questions/:id", async (req, res) => {
  const { text, options, correctAnswer, adminName, adminEmail } = req.body;
  // Use the same validation as for adding a question
  if (!text || typeof text !== "string" || !text.trim()) {
    return res.status(400).json({ message: "Question text is required." });
  }
  if (
  ...
  try {
    const question = await Question.findByIdAndUpdate(
      req.params.id,
      { text, options, correctAnswer, adminName, adminEmail },
